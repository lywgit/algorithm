...
int FindAndSetLeader(vector<int> & parent, int i){
    if (parent[i] == i) 
        return i;
    else
        return parent[i] = FindAndSetLeader(parent, parent[i]);
}
...
void KClustering(int k, vector<Edge> edges, int num_node, int & max_spacing){
    ...
    while (iter != edges.end()){
        // Find
        int s1 = FindAndSetLeader(parent, iter->node1);
        int s2 = FindAndSetLeader(parent, iter->node2); 
        if (s1 != s2) {
            // Union
            if (rank[s1] >= rank[s2]){
                parent[s2] = s1;
